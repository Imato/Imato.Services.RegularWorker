alter table workers 
    add started timestamp null;

CREATE OR REPLACE FUNCTION public.set_worker
  (_id integer, _name text, _app text, _appfullname text, _host text, _date timestamp without time zone, _active boolean, _settings text, _executed timestamp without time zone, _error text, _started timestamp without time zone)
 RETURNS table
  (id integer, name character varying, host character varying, app character varying, appfullname character varying, date timestamp without time zone, settings text, active boolean, executed timestamp without time zone, hosts integer, error text, activehosts integer, started timestamp without time zone)
 LANGUAGE plpgsql
AS $function$
declare 
  _hosts int = 0;
